...
- 评：可以把组件拖入视图，可以选中蓝色框，出现橙色框，可以拖动选中的器件。无引脚表示，NE555左边引脚排列正常，右边排列分散。只有表示引脚的点，没有电阻、电容、电源、地、芯片等图像。只能选中蓝色框附近，点击器件中心无法选中。无法框选。

## 鼠标交互

- [✔] 鼠标移动 → StatusBar mouse mm 列实时更新
- [X] 滚轮缩放 → StatusBar zoom% 更新
* 百分比没有更新
- [✔X] 右下角 +/−/100%/Fit/网格 按钮点击 → 符合直觉
* 点击- +有效，但是百分比没有更新
- [✔] 中键拖动 → 画布 pan
* 画布能拖动，但是上面的元器件拖到中间Viwe外面时不会被遮挡，会显示在其他组件上方
- [✔] 鼠标悬停 placement → 黄色描边
